<template>
  <div class="min-h-[calc(100vh-80px)] bg-[#f1f5f9] flex flex-col justify-between py-10 px-4 sm:px-6 lg:px-8">
    <div class="max-w-5xl w-full mx-auto bg-white rounded-3xl shadow-2xl border border-zinc-200 overflow-hidden grid grid-cols-1 lg:grid-cols-12 text-left">
      <!-- Left Form Pane -->
      <div class="lg:col-span-6 p-8 sm:p-12 flex flex-col justify-between space-y-6">
        <div>
          <!-- Mode Tabs -->
          <div class="flex items-center justify-between">
            <div class="inline-flex rounded-full bg-zinc-100 p-1 border border-zinc-200 text-xs font-bold">
              <button
                type="button"
                @click="mode = 'login'"
                class="px-4 py-1.5 rounded-full transition-all"
                :class="mode === 'login' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-800'"
              >
                ➔] Đăng nhập
              </button>
              <button
                ...
                :class="mode === 'register' ? 'bg-amber-400 text-zinc-900 font-bold shadow-sm' : 'text-zinc-500 hover:text-zinc-800'"
              >
                <span>Đăng ký doanh nghiệp</span>
                <span class="text-[9px] bg-amber-200 text-amber-900 px-1.5 py-0.2 rounded font-extrabold">14 ngày thử</span>
              </button>
            </div>
          ...
          <div class="mt-6">
            ...
            <h2 class="text-2xl sm:text-3xl font-extrabold text-zinc-900 mt-1">
          ...
          <div class="grid grid-cols-2 gap-3 mt-6">
            <button
              type="button"
              class="flex items-center justify-center gap-2 p-2.5 rounded-xl border border-zinc-200 hover:bg-zinc-50 text-xs font-semibold text-zinc-700 transition-colors"
            >
          ...
          <div class="relative my-6 text-center">
            ...
            <span class="relative bg-white px-3 text-[11px] text-zinc-400 uppercase font-medium">
          ...
          <form @submit.prevent="handleAuth" class="space-y-4 text-xs">
            ...
            <div>
              <label class="block font-bold text-zinc-700 mb-1">Email công việc hoặc Số điện thoại *</label>
              <input v-model="identifier" required placeholder="ten@doanhnghiep.com hoặc 0912xxx" class="w-full px-4 py-2.5 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-amber-500 outline-none" />
            </div>
            ...
            <div>
              ...
              <div class="relative">
                <input 
                  ...
                  required 
                  placeholder="••••••••••••" 
                  class="w-full px-4 py-2.5 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-amber-500 outline-none pr-10" 
                />
                <button 
                  type="button" 
                  @click="showPassword = !showPassword" 
                  class="absolute right-3 top-2.5 text-zinc-400 hover:text-zinc-600 focus:outline-none"
                >
                  {{ showPassword ? '🙈' : '👁️' }}
                </button>
              </div>
            ...
            <div class="flex items-center justify-between pt-1">
              <label class="flex items-center gap-2 cursor-pointer text-zinc-600">
                <input type="checkbox" v-model="rememberMe" class="rounded text-amber-600" />
            ...
            <button
              type="submit"
              class="w-full py-3.5 rounded-xl bg-amber-400 hover:bg-amber-400 text-zinc-900 font-bold font-extrabold text-sm shadow-md transition-all flex items-center justify-center gap-2"
            >
          ...
          <div class="mt-4 text-center text-xs text-zinc-500">
            ...
            <button 
              ...
              class="font-bold text-amber-600 hover:underline ml-1"
            >
        ...
        <!-- Security Badges -->
        <div class="pt-6 border-t border-zinc-100 flex items-center justify-between text-[11px] text-zinc-400">
      ...
      <div class="lg:col-span-6 bg-gradient-to-br from-[#0284c7] to-[#0369a1] p-8 sm:p-12 text-white flex flex-col justify-between space-y-8">
        <div class="space-y-6">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-xs font-semibold text-amber-100">
          ...
          <p class="text-amber-100 text-xs sm:text-sm leading-relaxed font-light">
          ...
          <div class="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 text-center">
            <div>
              ...
              <div class="text-[10px] text-amber-100 uppercase">Doanh nghiệp vận hành</div>
            </div>
            <div>
              <div class="text-xl sm:text-2xl font-black">1.2M+</div>
              <div class="text-[10px] text-amber-100 uppercase">Kiện hàng/tháng</div>
            </div>
            <div>
              <div class="text-xl sm:text-2xl font-black">99.98%</div>
              <div class="text-[10px] text-amber-100 uppercase">Độ chính xác vị trí</div>
            </div>
          </div>

          <!-- Featured Testimonial Card -->
          <div class="p-4 rounded-2xl bg-white text-zinc-800 shadow-lg space-y-2 text-xs">
            <div class="flex justify-between items-center">
              <span class="text-amber-400">★★★★★</span>
              <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded">Case Study: Y-Style Retail</span>
            </div>
            ...
            <div class="pt-1 flex items-center justify-between text-[11px] text-zinc-500">
              <span class="font-bold text-zinc-900">Trần Nhật Nam — Giám đốc Vận hành (COO)</span>
              <span class="text-amber-600">✓ Đã xác thực</span>
            </div>
          </div>
        </div>

        <!-- Footnote guarantees -->
        <div class="pt-4 border-t border-white/20 flex flex-wrap items-center justify-between text-[11px] text-amber-100">
