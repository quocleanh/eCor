<template>
  <div v-if="currentModule" class="bg-white min-h-screen">
    ...
    <div class="border-b border-zinc-200 bg-zinc-50/70 py-4">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex items-center gap-2 text-xs text-zinc-500 font-medium">
          ...
          <span>/</span>
          <span class="text-zinc-900 font-bold">{{ currentModule.title }}</span>
        </nav>
      </div>
    </div>

    <!-- 2. Hero Section Của Module -->
    <section class="py-14 md:py-20 border-b border-zinc-200 bg-white">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider mb-4"
               :class="isCoreModule ? 'bg-amber-100 text-amber-800' : 'bg-zinc-100 text-zinc-700'">
          ...
          <p class="text-base sm:text-lg text-zinc-600 leading-relaxed mb-8">
    ...
    <div v-if="isCoreModule">
      ...
      <section class="py-14 bg-zinc-50 border-b border-zinc-200">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="max-w-2xl mb-10">
            <div class="text-xs font-bold text-amber-700 uppercase tracking-wider mb-1">Thực Trạng Vận Hành</div>
            <h2 class="text-2xl font-bold text-zinc-900">Các Bài Toán Nút Thắt eCor {{ currentModule.code }} Giải Quyết</h2>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div v-for="(pain, idx) in currentModule.pains" :key="idx" class="p-5 rounded-xl bg-white border border-zinc-200">
              <div class="text-2xl mb-3">{{ pain.icon }}</div>
              <h3 class="text-sm font-bold text-zinc-900 mb-2">{{ pain.title }}</h3>
              <p class="text-xs text-zinc-600 leading-relaxed">{{ pain.desc }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="py-16 bg-white border-b border-zinc-200">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="max-w-2xl mb-12">
            <div class="text-xs font-bold text-amber-700 uppercase tracking-wider mb-1">Nghiệp Vụ Thực Chiến</div>
            <h2 class="text-2xl font-bold text-zinc-900">Tính Năng Cốt Lõi Được Xây Dựng Cho Người Vận Hành</h2>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="(feat, idx) in currentModule.features" :key="idx" class="p-5 rounded-xl border border-zinc-200 bg-zinc-50/50 hover:border-amber-300 transition-colors">
              <h3 class="text-sm font-bold text-zinc-900 mb-2 flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full bg-amber-600"></span>
                {{ feat.title }}
              </h3>
              <p class="text-xs text-zinc-600 leading-relaxed">{{ feat.desc }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="py-16 bg-amber-50/60 border-b border-amber-200/60">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="max-w-2xl mb-8">
            <div class="text-xs font-bold text-amber-700 uppercase tracking-wider mb-1">Vòng Khép Kín Dữ Liệu</div>
            <h2 class="text-2xl font-bold text-zinc-900">Cách {{ currentModule.code }} Đồng Bộ Liền Mạch</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div v-for="(integ, idx) in currentModule.integrations" :key="idx" class="p-5 rounded-xl bg-white border border-amber-200">
              <div class="text-xs font-bold text-amber-700 mb-1">&rarr; {{ integ.label }}</div>
              <h3 class="text-sm font-bold text-zinc-900 mb-2">{{ integ.desc.split(':')[0] || integ.label }}</h3>
              <p class="text-xs text-zinc-600 leading-relaxed">{{ integ.desc }}</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <div v-else>
      ...
      <section class="py-14 bg-zinc-50 border-b border-zinc-200">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="max-w-2xl mb-8">
            <div class="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1">Tính Năng Chính</div>
            <h2 class="text-2xl font-bold text-zinc-900">Năng Lực Mở Rộng Của Phân Hệ {{ currentModule.code }}</h2>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div v-for="(feat, idx) in currentModule.features.slice(0, 4)" :key="idx" class="p-5 rounded-xl bg-white border border-zinc-200">
              <h3 class="text-sm font-bold text-zinc-900 mb-2">{{ feat.title }}</h3>
              <p class="text-xs text-zinc-600 leading-relaxed">{{ feat.desc }}</p>
            </div>
          </div>
        </div>
      ...
      <section class="py-12 bg-white border-b border-zinc-200">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="p-6 rounded-xl bg-zinc-50 border border-zinc-200 max-w-3xl">
            <h3 class="text-sm font-bold text-amber-700 mb-2">Tích hợp tự động vào 3 trụ cột lõi (POS - WMS - Kế toán)</h3>
    ...
    <section class="py-16 bg-white border-b border-zinc-200">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-xl font-bold text-zinc-900 mb-6 text-center">Câu Hỏi Thường Gặp Về eCor {{ currentModule.code }}</h2>
        <FaqAccordion :faqs="currentModule.faqs" class="mb-12" />

        <div class="p-8 rounded-2xl bg-zinc-900 text-white text-center">
          <h3 class="text-xl font-bold mb-2">Sẵn Sàng Triển Khai {{ currentModule.code }} Cho Cửa Hàng Của Bạn?</h3>
          <p class="text-xs text-zinc-400 max-w-lg mx-auto mb-6">
            Đội ngũ chuyên gia eCor sẵn sàng khảo sát bài toán thực tế và thiết lập dữ liệu mẫu ngay trong ngày.
          </p>
          <button @click="openModal" class="px-6 py-3 text-xs font-bold text-zinc-900 font-bold bg-amber-400 hover:bg-amber-400 rounded-lg transition-colors">
